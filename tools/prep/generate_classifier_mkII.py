...
def defineArguments():
    ...
    parser.add_argument("-l", "--labeling", help="The labeling scheme to adopt", default="vt1-vt1", choices=["vt1-vt1", "vt50p-vt50p", "vt50p-vt1"], required=False)
    return parser
...
def main():
    try:
        ...
        for f in allApps:
            ...
            if not os.path.exists("%s/%s" % (VT_REPORTS_DIR, f[f.rfind("/")+1:].replace(".%s" % arguments.featuretype, ".report"))):
